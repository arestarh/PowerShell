...
if ($force -eq $true -and (Test-Path -Path $destpath) -eq $true) {
    ...
    if ($soursepath -eq (Resolve-Path -Path $destpath).Path) {
        Write-Error -Message "Be aware, that you've specified the same path for sourse and destination and Force parameter is used:`n$destpath.`nTo avoid errors, script execution was aborted.`nPlease, use another path for destination .zip file."
        Return
    }
    else {
        try {
            ...
            Write-Error -Message "Error has occurred while forcing file deletion: $destpath"
...
if ($force -eq $false -and (Test-Path -Path $destpath) -eq $true) {
    Write-Error -Message "Archive File $destpath already exists: please, delete file or use Force parameter"
    Return
}
...
try {
    ...
    if ($destpathitem.IsReadOnly -eq $true) {
        $destpathitem.IsReadOnly = $false
    }
}
catch {
    Write-Error -Message "Error has occurred while setting destination path"
    Write-Verbose -Message $_ -Verbose
    Return
}
...
try {
    ...
    Write-Error -Message "Error has occurred while invoking CopyHere method (copying an item or items to a folder): $soursepath"
    Write-Verbose -Message $_ -Verbose
    Return
}
